...
@section('title', 'إضافة حقل جديد - ' . $category->name)
...
<div class="container-fluid">
        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">إضافة حقل</li>
                </ol>
            </nav>
        </div>
    ...
    <form action="{{ route('admin.categories.fields.store', $category) }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                           ...
                           {{ isset($field) && $field->id ? '' : 'readonly' }}>
                    <small class="text-muted">
                        <i class="fas fa-info-circle"></i>
                        @if(isset($field) && $field->id)
                            يمكن تعديل المفتاح في حالة التعديل
                        @else
                            سيتم إنشاء المفتاح تلقائياً من الاسم الإنجليزي
                        @endif
                    </small>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <small class="text-muted">
                        <i class="fas fa-info-circle"></i>
                        رقم الترتيب (0، 1، 2، ...). الأرقام الأقل تظهر أولاً. اتركه فارغاً ليتم ترتيبه تلقائياً.
                    </small>
                    @error('sort_order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
        ...
        <div class="mt-3">
            ...
            html = `
                <div class="card">
                    <div class="card-body">
                        <label class="form-label"><i class="fas fa-star"></i> القيمة الافتراضية:</label>
                        <input type="file" name="value" class="form-control" accept="image/*" ${value ? '' : 'required'}>
                        <small class="text-muted">
